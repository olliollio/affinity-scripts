...
module.exports = function (PD, h) {
  ...
  h.assertClose('CCW square is +area', PD.signedArea(SQUARE_CCW), 100);
  h.assertClose('CW square is -area', PD.signedArea(SQUARE_CW), -100);
  h.assertClose('triangle is half the box', PD.signedArea([0, 0, 4, 0, 0, 3]), 6);
  h.assertClose('degenerate line has no area', PD.signedArea([0, 0, 5, 0, 10, 0]), 0);
  h.assertClose('fewer than 3 points has no area', PD.signedArea([0, 0, 5, 5]), 0);
  h.assertClose('translation does not change area',
    PD.signedArea([100, 50, 110, 50, 110, 60, 100, 60]), 100);
  ...
  h.assert('centre of a square is inside', PD.pointInRing(SQUARE_CCW, 5, 5) === true);
  h.assert('outside the bbox is outside', PD.pointInRing(SQUARE_CCW, 20, 5) === false);
  h.assert('inside the bbox but past an edge is outside', PD.pointInRing(SQUARE_CCW, -1, 5) === false);
  h.assert('winding does not affect containment', PD.pointInRing(SQUARE_CW, 5, 5) === true);
  h.assert('concave notch is outside', PD.pointInRing(C_SHAPE, 8, 5) === false);
  h.assert('concave arm is inside', PD.pointInRing(C_SHAPE, 8, 1.5) === true);
  h.assert('spine of the C is inside', PD.pointInRing(C_SHAPE, 2, 5) === true);
  h.assert('a ray grazing a vertex still resolves',
    PD.pointInRing([0, 0, 10, 0, 5, 10], 5, 5) === true);
  ...
  function faceBox(face) { return PD.signedArea(face.outer); }

  var one = PD.buildFaces([box(0, 0, 100, 100)]);
  ...
  var ring = PD.buildFaces([box(0, 0, 100, 100), box(20, 20, 80, 80)]);
  ...
  var ringSwapped = PD.buildFaces([box(20, 20, 80, 80), box(0, 0, 100, 100)]);
  ...
  var b = PD.buildFaces([box(0, 0, 100, 100), box(20, 10, 80, 40), box(20, 60, 80, 90)]);
  ...
  var i2 = PD.buildFaces([box(0, 0, 10, 60), box(0, 70, 10, 80)]);
  ...
  var island = PD.buildFaces([box(0, 0, 100, 100), box(20, 20, 80, 80), box(40, 40, 60, 60)]);
  ...
  h.assertClose('outermost hole is the depth-1 ring', Math.abs(PD.signedArea(deep[0].holes[0])), 6400);
  h.assertEqual('the island holds its own hole', deep[1].holes.length, 1);
  h.assertClose('island hole is the depth-3 ring', Math.abs(PD.signedArea(deep[1].holes[0])), 1600);

  // Input winding is whatever the source produced; classification must not depend on it.
  var mixed = PD.buildFaces([reversed(box(0, 0, 100, 100)), box(20, 20, 80, 80)]);
  ...
  h.assertEqual('no rings, no faces', PD.buildFaces([]).length, 0);
  h.assertEqual('rings under 3 points are ignored', PD.buildFaces([[0, 0, 5, 5]]).length, 0);
};
