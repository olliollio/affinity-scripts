...
module.exports = function (GR, h) {
  ...
  function run(face) {
    var clean = GR.sanitizeFace(face);
    return { clean: clean, parts: GR.decompose(clean) };
  }
  ...
  inv.assertInvariants(GR, h, 'square', squareRun.clean, squareParts);
  ...
  inv.assertInvariants(GR, h, 'triangle', triRun.clean, triParts);
  ...
  inv.assertInvariants(GR, h, 'L', lRun.clean, lParts);
  ...
  inv.assertInvariants(GR, h, 'comb', combRun.clean, combParts);

  h.group('decompose: holes');

  // "O" — the case physicsdrop gets wrong by hulling it into a solid slab.
  ...
  inv.assertInvariants(GR, h, 'O', ringRun.clean, ringParts);
  ...
  inv.assertInvariants(GR, h, 'B', bRun.clean, bParts);
  ...
  inv.assertInvariants(GR, h, 'odd counter', oddRun.clean, oddParts);
  ...
  var needed = Math.ceil(24 / GR.MAX_VERTS);
  h.assert('a 24-gon is split by the cap alone', discParts.length >= needed,
    'got ' + discParts.length + ', cap ' + GR.MAX_VERTS + ' needs at least ' + needed);
  inv.assertInvariants(GR, h, '24-gon', discRun.clean, discParts);

  var capped = GR.decompose(disc, { maxVerts: 4 });
  var worst = 0;
  ...
  inv.assertInvariants(GR, h, '24-gon capped', GR.sanitizeFace(disc, { maxVerts: 4 }), capped, { maxVerts: 4 });

  h.group('decompose: degenerate input');

  h.assertEqual('a collapsed outer yields no parts', GR.decompose({ outer: [0, 0, 5, 0, 10, 0], holes: [] }).length, 0);
  h.assertEqual('an empty face yields no parts', GR.decompose({ outer: [], holes: [] }).length, 0);
  h.assertEqual('a missing face yields no parts', GR.decompose(null).length, 0);

  h.group('invariants: negative control');

  // The invariants are only worth their runtime if they actually fire. Feed them the physicsdrop failure
  ...
  h.assert('area conservation rejects an ignored hole',
    Math.abs(inv.partsArea(GR, slab) - inv.faceArea(GR, ringRun.clean)) > 1e-3 * inv.faceArea(GR, ringRun.clean));
  var leak = inv.checkHoleExclusion(GR, ringRun.clean, slab);
  ...
  var goodSamples = inv.checkHoleExclusion(GR, ringRun.clean, ringParts);
  ...
  h.assert('convexity rejects a reflex part',
    inv.checkConvexPart(GR, [0, 0, 60, 0, 60, 20, 20, 20, 20, 60, 0, 60], 8).ok === false);
  h.assert('convexity rejects a part over the cap',
    inv.checkConvexPart(GR, poly, 8).ok === false);
  h.assert('convexity rejects negative winding',
    inv.checkConvexPart(GR, [0, 0, 0, 10, 10, 10, 10, 0], 8).ok === false);
  h.assert('convexity accepts a clean quad',
    inv.checkConvexPart(GR, box(0, 0, 10, 10), 8).ok === true);
  ...
  for (var k = 0; k < 60; k++) {
    ...
    var face = { outer: GR.enforceWinding(starPolygon(rand, n, 12, 45), true), holes: [] };
    var r = run(face);
    maxParts = Math.max(maxParts, r.parts.length);

    var want = inv.faceArea(GR, r.clean);
    var got = inv.partsArea(GR, r.parts);
    if (Math.abs(got - want) > 1e-3 * want) { failures++; continue; }
    for (var p = 0; p < r.parts.length; p++) {
      if (!inv.checkConvexPart(GR, r.parts[p], GR.MAX_VERTS).ok) { failures++; break; }
    }

    // Separately: what sanitising cost. Simplification is allowed to move area, but only a little.
    var raw = inv.faceArea(GR, face);
