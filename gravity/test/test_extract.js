...
module.exports = function (GR, h) {
  ...
  ['wall', 'Wall', 'FLOOR', 'left-wall', 'Wall 3', 'the ramp', 'static', 'ground',
   'collider', 'Collider', 'big collider', 'collider-2'].forEach(function (n) {
    h.assert('"' + n + '" is scenery', GR.isStaticName(n) === true);
  });
  ...
  ['Wallpaper', 'flooring', 'rampage', 'ecstatic', 'background', 'Walls0', 'colliders',
   'collide'].forEach(function (n) {
    h.assert('"' + n + '" is not scenery', GR.isStaticName(n) === false);
  });
  h.assert('an empty name is not scenery', GR.isStaticName('') === false);
  h.assert('a missing name is not scenery', GR.isStaticName(null) === false);

  h.group('extract: classification');

  h.assertEqual('a curve node is vector',
    GR.classifyNode(mockNode({ curves: [mockCurve(boxBeziers(0, 0, 10, 10))] })), 'vector');
  h.assertEqual('a text node is text', GR.classifyNode(mockNode({ tag: 'ArtTextNode' })), 'text');
  h.assertEqual('an image node is image', GR.classifyNode(mockNode({ tag: 'ImageNode' })), 'image');
  h.assertEqual('a group is a group', GR.classifyNode(mockNode({ tag: 'GroupNode' })), 'group');
  h.assertEqual('nothing is none', GR.classifyNode(null), 'none');
  ...
  h.assertEqual('an image with curves is still an image', GR.classifyNode(imgWithCurves), 'image');
  var textWithCurves = mockNode({ tag: 'ArtTextNode', curves: [mockCurve(boxBeziers(0, 0, 10, 10))] });
  h.assertEqual('text with curves is still text', GR.classifyNode(textWithCurves), 'text');

  h.group('extract: rings and transform');

  var plain = mockNode({ curves: [mockCurve(boxBeziers(0, 0, 100, 50))] });
  var rings = GR.ringsOf(plain);
  ...
  var movedRings = GR.ringsOf(moved);
  ...
  var scaledRings = GR.ringsOf(scaledNode);
  h.assertClose('a scaling transform reaches the far corner x',
    scaledRings[0][2], 599 * 0.5976 + 236.6482, 1e-6);

  h.assertEqual('matrixOf reads the row-major data', GR.matrixOf(moved).join(','), '1,0,220,0,1,228');
  h.assertEqual('matrixOf on a node without one is null', GR.matrixOf({}), null);

  // An open path has no interior, so it cannot become a body.
  var open = mockNode({ curves: [mockCurve(boxBeziers(0, 0, 100, 50), false)] });
  h.assertEqual('an open curve is skipped', GR.ringsOf(open).length, 0);
  h.assertEqual('unless explicitly included', GR.ringsOf(open, { includeOpen: true }).length, 1);
  ...
  var conv = GR.convertTextToCurves(null, [textNode]);
  ...
  h.assertEqual('with no text nodes it is a no-op',
    GR.convertTextToCurves(null, [mockNode({ curves: [mockCurve(boxBeziers(0, 0, 10, 10))] })]).converted, 0);
  ...
  var merged3 = GR.mergeNodeLists([keepA, keepB], [newC]);
  ...
  h.assertEqual('a node present in both lists appears once',
    GR.mergeNodeLists([keepA, keepB], [keepB, newC]).length, 3);
  ...
  h.assert('two wrappers with one handle are one node', GR.sameNode(wrapper1, wrapper2) === true);
  h.assertEqual('and merge to a single entry', GR.mergeNodeLists([wrapper1], [wrapper2]).length, 1);
  h.assert('different handles stay distinct',
    GR.sameNode({ handle: 'h7' }, { handle: 'h8' }) === false);
  h.assert('isSameNode wins when present',
    GR.sameNode({ handle: 'a', isSameNode: function () { return true; } }, { handle: 'b' }) === true);

  h.assertEqual('empty lists merge to nothing', GR.mergeNodeLists([], []).length, 0);
  h.assertEqual('missing lists are tolerated', GR.mergeNodeLists(null, null).length, 0);
  h.assertEqual('nulls inside a list are dropped', GR.mergeNodeLists([null, keepA], [null]).length, 1);
  ...
  var oneBody = GR.extract([group], { groupsAsOneBody: true });
  ...
  h.assert('a named wall is static', GR.extract([wall]).objects[0].isStatic === true);

  var lockedThing = mockNode({ name: 'Curves', locked: true, curves: [mockCurve(boxBeziers(0, 0, 10, 10))] });
  h.assert('a locked node is static', GR.extract([lockedThing]).objects[0].isStatic === true);

  var loose = mockNode({ name: 'Curves', curves: [mockCurve(boxBeziers(0, 0, 10, 10))] });
  h.assert('an ordinary node is dynamic', GR.extract([loose]).objects[0].isStatic === false);
  ...
  h.assert('an ordinary group leaves its children dynamic',
    GR.extract([plainGroup]).objects[0].isStatic === false);
  ...
  h.assertEqual('or is refused on request', GR.extract([img], { imagePolicy: 'refuse' }).refusals.length, 1);

  h.group('extract: end to end');

  // The real contract: a selection goes in, planck fixtures come out.
  var W = GR.makeWorld({ scale: 100 });
  var out = GR.extract([letterO]);
  var parts = GR.decompose(out.objects[0].faces[0]);
  var body = GR.addBody(W, parts, {});
