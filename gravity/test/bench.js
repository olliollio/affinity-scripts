...
var GR = h.loadPD(['contours.js', 'sanitize.js', 'decompose.js']);
...
function measure(face, opts) {
  ...
  var clean = GR.sanitizeFace(copy, o);
  if (!clean) return { parts: 0, cost: 0, verts: 0 };
  var parts = GR.decompose(clean, o);
  var raw = inv.faceArea(GR, face);
  var got = inv.faceArea(GR, clean);
  return {
    parts: parts.length,
...
console.log('gravity geometry benchmark   MAX_VERTS = ' + GR.MAX_VERTS);
console.log('');
