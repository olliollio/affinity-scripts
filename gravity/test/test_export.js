 ...
 * than failing outright, the way physicsdrop would.
 */
...
module.exports = function (GR, h) {
  ...
  var noFs = GR.exportResolveTarget(mockApp, 'png');
  ...
  h.assert('and the failure names the folder it wanted',
    noFs.error.indexOf('Gravity_') >= 0, noFs.error);
  ...
  h.assert('the intended folder uses forward slashes',
    /E:\\USER\\Desktop\/Gravity_/.test(noFs.error), noFs.error);
  h.assert('and appends no backslash of its own',
    noFs.error.indexOf('Desktop\\Gravity') < 0, noFs.error);
  ...
  var name = GR.exportFrameName(7, 'png');
  h.assertEqual('a frame name is zero padded to four digits', name, 'drop_0007.png');
  h.assertEqual('and stays padded past nine', GR.exportFrameName(42, 'png'), 'drop_0042.png');
  h.assertEqual('and does not overflow at four digits', GR.exportFrameName(1234, 'png'), 'drop_1234.png');
  h.assertEqual('jpeg gets the jpg extension', GR.exportFrameName(0, 'jpg'), 'drop_0000.jpg');

  h.group('export: failure reporting');

  h.assert('no Desktop path is reported rather than thrown',
    !!GR.exportResolveTarget({}, 'png').error);
  h.assert('an app that throws is reported too',
    !!GR.exportResolveTarget({ get userDesktopPath() { throw new Error('denied'); } }, 'png').error);
  ...
  var reported = null;
  GR.exportSequence({ doc: null, lastIndex: 3 }, {}, function (res) { reported = res; });
