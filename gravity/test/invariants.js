 ...
 *                           Catches "holes silently ignored", which is exactly the physicsdrop bug.
 *
...
function ringArea(GR, ring) { return Math.abs(GR.signedArea(ring)); }

// Net solid area a face should have once decomposed.
function faceArea(GR, face) {
  var a = ringArea(GR, face.outer);
  var holes = face.holes || [];
  for (var i = 0; i < holes.length; i++) a -= ringArea(GR, holes[i]);
  return a;
}

function partsArea(GR, parts) {
  var a = 0;
  for (var i = 0; i < parts.length; i++) a += ringArea(GR, parts[i]);
  return a;
}
 ...
 */
function sampleInside(GR, hole, steps, margin) {
  var n = hole.length >> 1;
  ...
  for (var gx = 1; gx < steps; gx++) {
    for (var gy = 1; gy < steps; gy++) {
      ...
      if (!GR.pointInRing(hole, px, py)) continue;
 ...
 */
function checkHoleExclusion(GR, face, parts, opts) {
  var o = opts || {};
  var steps = o.steps || 7;
  var holes = face.holes || [];
  var checked = 0;
  for (var i = 0; i < holes.length; i++) {
    var margin = o.margin !== undefined ? o.margin : Math.sqrt(Math.abs(GR.signedArea(holes[i]))) * 0.02;
    var pts = sampleInside(GR, holes[i], steps, margin);
    for (var s = 0; s < pts.length; s++) {
      checked++;
      for (var p = 0; p < parts.length; p++) {
        if (GR.pointInRing(parts[p], pts[s][0], pts[s][1])) {
          return {
            ok: false,
            samples: checked,
 ...
 */
function assertInvariants(GR, h, label, face, parts, opts) {
  var o = opts || {};
  var maxVerts = o.maxVerts || GR.MAX_VERTS;
  var tol = o.areaTol || 1e-3; // 0.1% relative

  var want = faceArea(GR, face);
  var got = partsArea(GR, parts);
  ...
  for (var i = 0; i < parts.length && !bad; i++) {
    var res = checkConvexPart(GR, parts[i], maxVerts);
  ...
  if (holes) {
    var ex = checkHoleExclusion(GR, face, parts, o);
