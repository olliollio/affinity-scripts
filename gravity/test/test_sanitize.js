...
module.exports = function (GR, h) {
  ...
  var dup = GR.sanitizeRing([0, 0, 0, 0, 10, 0, 10, 10, 0, 10]);
  h.assertEqual('duplicate consecutive point is dropped', ringLen(dup), 4);
  h.assertClose('duplicate does not change area', GR.signedArea(dup), 100);

  var nearDup = GR.sanitizeRing([0, 0, 1e-9, 1e-9, 10, 0, 10, 10, 0, 10]);
  h.assertEqual('sub-epsilon neighbour is merged', ringLen(nearDup), 4);

  var wrapDup = GR.sanitizeRing([0, 0, 10, 0, 10, 10, 0, 10, 0, 0]);
  h.assertEqual('closing point equal to the first is dropped', ringLen(wrapDup), 4);

  var collinear = GR.sanitizeRing([0, 0, 5, 0, 10, 0, 10, 10, 0, 10]);
  h.assertEqual('midpoint of a straight run is dropped', ringLen(collinear), 4);
  h.assertClose('collinear removal does not change area', GR.signedArea(collinear), 100);

  var wrapCollinear = GR.sanitizeRing([5, 0, 10, 0, 10, 10, 0, 10, 0, 0]);
  h.assertEqual('collinear vertex at the wrap point is dropped', ringLen(wrapCollinear), 4);

  var manyCollinear = GR.sanitizeRing([0, 0, 1, 0, 2, 0, 3, 0, 10, 0, 10, 10, 0, 10]);
  ...
  h.assert('an all-collinear ring is rejected',
    GR.sanitizeRing([0, 0, 5, 0, 10, 0, 5, 0]) === null);
  h.assert('a zero-area sliver is rejected',
    GR.sanitizeRing([0, 0, 10, 0, 10, 1e-12, 0, 1e-12]) === null);
  h.assert('a ring that collapses below 3 points is rejected',
    GR.sanitizeRing([0, 0, 1e-12, 0, 0, 1e-12]) === null);
  h.assert('an empty ring is rejected', GR.sanitizeRing([]) === null);

  var input = [0, 0, 0, 0, 10, 0, 10, 10, 0, 10];
  GR.sanitizeRing(input);
  h.assertEqual('the input ring is not mutated', input.length, 10);

  var clean = [0, 0, 10, 0, 10, 10, 0, 10];
  var cleaned = GR.sanitizeRing(clean);
  ...
  arc.push(0, 0);
  var keptArc = GR.sanitizeRing(arc);
  ...
  var madeCcw = GR.enforceWinding(cw, true);
  h.assertClose('a clockwise ring is flipped to positive', GR.signedArea(madeCcw), 100);
  h.assertEqual('flipping keeps every point', ringLen(madeCcw), 4);
  h.assertEqual('the source ring is not mutated', GR.signedArea(cw), -100);

  var ccw = [0, 0, 10, 0, 10, 10, 0, 10];
  h.assertClose('a ring already positive is left alone', GR.signedArea(GR.enforceWinding(ccw, true)), 100);
  h.assertClose('asking for negative flips a positive ring',
    GR.signedArea(GR.enforceWinding(ccw, false)), -100);
  h.assertClose('asking for negative leaves a negative ring',
    GR.signedArea(GR.enforceWinding(cw, false)), -100);
  ...
  function ringArea(r) { return Math.abs(GR.signedArea(r)); }

  var bumped = GR.simplifyRing([0, 0, 50, 0.05, 100, 0, 100, 100, 0, 100], { simplifyTol: 0.25 });
  h.assertEqual('a bump under tolerance is flattened away', ringLen(bumped), 4);

  var kinked = GR.simplifyRing([0, 0, 50, 8, 100, 0, 100, 100, 0, 100], { simplifyTol: 0.25 });
  h.assertEqual('a real corner survives', ringLen(kinked), 5);

  var c120 = circleRing(250, 120);
  var cSimp = GR.simplifyRing(c120);
  ...
  h.assertEqual('the tolerance is scale-free',
    ringLen(GR.simplifyRing(circleRing(2500, 120))), ringLen(GR.simplifyRing(circleRing(250, 120))));

  // ...but never below the absolute floor, or small art dissolves into its own rounding noise.
  var tiny = GR.simplifyRing(circleRing(2, 120));
  ...
  h.assertEqual('a triangle is never simplified away',
    ringLen(GR.simplifyRing([0, 0, 100, 0, 50, 80], { simplifyTol: 10 })), 3);
  ...
  h.assert('a tolerance larger than the shape rejects it',
    GR.simplifyRing(circleRing(250, 120), { simplifyTol: 1e6 }) === null);
  ...
  var plain = GR.sanitizeFace({ outer: boxCw(0, 0, 100, 100), holes: [] });
  h.assertClose('the outer comes back positive', GR.signedArea(plain.outer), 10000);
  h.assertEqual('a face without holes keeps none', plain.holes.length, 0);

  var holed = GR.sanitizeFace({ outer: box(0, 0, 100, 100), holes: [box(20, 20, 80, 80)] });
  h.assertEqual('a real counter is kept', holed.holes.length, 1);
  h.assertClose('holes come back negative', GR.signedArea(holed.holes[0]), -3600);
  h.assertClose('net signed area is outer minus hole',
    GR.signedArea(holed.outer) + GR.signedArea(holed.holes[0]), 6400);

  // Absolute rule: 0.05 x 0.05 = 0.0025pt^2, below the 0.01pt^2 floor.
  var dust = GR.sanitizeFace({ outer: box(0, 0, 100, 100), holes: [box(50, 50, 50.05, 50.05)] });
  ...
  var bigGlyph = GR.sanitizeFace({ outer: box(0, 0, 1000, 1000), holes: [box(10, 10, 15, 15)] });
  h.assertEqual('a hole too small for its face is dropped', bigGlyph.holes.length, 0);

  var sameHoleSmallFace = GR.sanitizeFace({ outer: box(0, 0, 100, 100), holes: [box(10, 10, 15, 15)] });
  ...
  h.assert('a face below the absolute floor is dropped',
    GR.sanitizeFace({ outer: box(0, 0, 0.05, 0.05), holes: [] }) === null);
  h.assert('a face whose outer collapses is dropped',
    GR.sanitizeFace({ outer: [0, 0, 5, 0, 10, 0], holes: [] }) === null);

  var badHole = GR.sanitizeFace({ outer: box(0, 0, 100, 100), holes: [[0, 0, 5, 0, 10, 0]] });
  ...
  var mixedHoles = GR.sanitizeFace({
  ...
  h.assertEqual('a missing holes array is treated as none',
    GR.sanitizeFace({ outer: box(0, 0, 100, 100) }).holes.length, 0);

  var srcOuter = boxCw(0, 0, 100, 100);
  var srcHole = box(20, 20, 80, 80);
  GR.sanitizeFace({ outer: srcOuter, holes: [srcHole] });
  h.assertClose('the source outer is not rewound', GR.signedArea(srcOuter), -10000);
  h.assertClose('the source hole is not rewound', GR.signedArea(srcHole), 3600);

  var bigO = { outer: circleRing(250, 120), holes: [circleRing(40, 120, true)] };
  var simplified = GR.sanitizeFace(bigO);
  ...
  var wideClean = GR.sanitizeFace(wideRing);
  var holeAlone = GR.simplifyRing(circleRing(150, 120, true));
  ...
  var hairline = GR.sanitizeFace({ outer: [0, 0, 1000, 0, 1000, 0.2, 0, 0.2], holes: [] });
  ...
  var stemClean = GR.sanitizeFace(stem);
  ...
  h.assert('a smooth circle still simplifies hard', ringLen(GR.sanitizeFace(bigO).outer) < 80,
    'kept ' + ringLen(GR.sanitizeFace(bigO).outer));

  var unsimplified = GR.sanitizeFace(bigO, { simplify: false });
  ...
  var pass1 = GR.sanitizeFace(bigO);
  var pass2 = GR.sanitizeFace(pass1);
  ...
  h.assertClose('sanitising twice does not move area',
    GR.signedArea(pass2.outer), GR.signedArea(pass1.outer), 1e-9);
