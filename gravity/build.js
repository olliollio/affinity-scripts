/**
 * Build script for gravity.
 *
 *   node gravity/build.js            # writes gravity/dist/gravity.js
 *   node gravity/build.js --check    # verifies the committed output is up to date, exit 1 if not
 *
...
var OUT_FILE = path.join(OUT_DIR, 'gravity.js');

// Concatenation order is the only contract between modules: each src file is an IIFE over the
// shared GR namespace and may use anything defined above it.
var VENDOR = [
  // [file, GR key, how to pick the export off the private module]
...
function build() {
  ...
  parts.push(' *');
  parts.push(' * GENERATED FILE - do not edit. Built from gravity/src/ by gravity/build.js.');
  ...
  parts.push('else console.log(\'gravity ' + HEADER[2].split(': ')[1] +
...
if (process.argv.indexOf('--check') >= 0) {
  ...
  if (fs.readFileSync(OUT_FILE, 'utf8') !== code) {
    console.error('build --check: dist is stale, run `node gravity/build.js`');
    process.exit(1);
  }
