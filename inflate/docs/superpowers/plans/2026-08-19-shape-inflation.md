...
(function (GR) {
  ...
  function main() {
    // `app` is NOT a sandbox global, it is a module export — add_anchor_points and gravity both
    // reach it through /application, and the SDK reference lists it that way. Using a bare `app`
    // throws only when the first alert fires, which for a successful run is the SUCCESS alert at
    // the very end: every shape is already inflated and the script dies reporting it.
